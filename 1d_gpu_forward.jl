# ============================================================
# 1-D RF slice design — Enzyme JVP via timeline tangents
# Stable: Float64 kernel, SU(2) half-angle, Δf=0 during training
# Keeps phase-matched loss; no FiniteDiff.
# ============================================================
...
const zmax   = 8e-3
const fmax   = TBP / Trf
const γ64    = 2π * 42.57747892e6       # rad/(s·T), Float64
...
const Nrf = length(seq.RF[1].A)                # real-only controls
...
# =============================================================
# Device buffers (Float64 everywhere)
# =============================================================
const N         = length(z)
...
# State
M_xy  = adapt_dev(zeros(Float64, 2N))   # [1:N]=Re, [N+1:2N]=Im
M_z   = adapt_dev(ones(Float64, N))

# Spins/params
p_x   = adapt_dev(Float64.(z))
p_y   = adapt_dev(zeros(Float64, N))
p_z   = adapt_dev(zeros(Float64, N))
p_ΔBz = adapt_dev(zeros(Float64, N))
p_T1  = adapt_dev(fill(Float64(1e9), N))
p_T2  = adapt_dev(fill(Float64(1e9), N))
p_ρ   = adapt_dev(ones(Float64, N))
...
@kernel inbounds=true function excitation_kernel!(
    ...
    if i <= Int(N_Spins)
        ...
        @inbounds while s_idx <= Int(N_Δt)
            ...
            B  = sqrt(B2)
            φ  = T(0.5) * T(γ64) * B * Δt
            φ  = ifelse(isfinite(φ), φ, zero(T))   # drop rem2pi; keep an isfinite guard
            sφ = sin(φ)
            cφ = cos(φ)
            denom = B + eps_d
...
function loss_and_grad!(∇x::Vector{Float64}, x::Vector{Float64})
    ...
    # build ∂L/∂M on device (phase-matched) from the *current* M on device
    fill!(dM_xy, 0.0)
    ...
    CUDA.synchronize()
    dM_seed_host = Array(dM_xy)   # host copy for dot with JVPs
    ...
    for j in 1:n_ctrl
        ...
        # reset primal & tangent states on device
        fill!(M_xy, 0.0); fill!(M_z, 1.0)
...
bt = @benchmark train_loop_mm!(xloc, nst, clip; λ_init=$λ_init) setup=(xloc = zeros(Float64, n_ctrl); nst = $nsteps; clip = $rf_clip) evals=1 samples=20 seconds=30000
...
# @info "Running optimization loop for $nsteps steps..."
